...
blank_trans = Transition(0, torch.zeros(436, dtype=torch.uint8), None, 0, False)  # Value depends on state_size, now is args.prediction_depth * 4 + 4 TODO
...
class ReplayMemory():
	def __init__(self, args, capacity):
		...
		self.history = args.history_length  # Number of consecutive states processed
		self.discount = args.discount
		self.n = args.multi_step  # Number of steps for multi-step return
		self.priority_weight = args.priority_weight  # Initial importance sampling weight β, annealed to 1 over course of training
		self.priority_exponent = args.priority_exponent  # Prioritised experience replay exponent (α)
	...
	def append(self, state, action, reward, terminal):
		...
		state = state.to(dtype=torch.uint8, device=torch.device('cpu'))  # Discretize to save memory
